...
import {
  ...
  FiInfo, FiCheckCircle, FiXCircle, FiClock, FiMousePointer, FiCalendar,
  FiChevronLeft, FiChevronRight, FiSun, FiMoon, FiCloud, FiStar, FiHeart,
  FiGift, FiArrowLeft, FiPlus, FiTrash2, FiFileText, FiDownload, FiEye
} from 'react-icons/fi';
...
const AddResourceForm = () => {
  ...
  const getCategoriesByType = () => {
    if (formData.type === 'product') {
      ...
      return [
        { value: 'salle', label: 'Salle', icon: FiMapPin },
        { value: 'traiteur', label: 'Traiteur', icon: FiUsers }
      ];
    }
  };
  ...
  useEffect(() => {
    let filled = 0;
    let total = 6; // name, description, price, type, category, termsText
    ...
    <div className={`min-h-screen ${getBackgroundClass()} p-6 relative overflow-hidden transition-colors duration-1000`}>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Colonne gauche - Formulaire modernisé */}
                ...
                <div className="grid grid-cols-2 gap-3">
                  {getCategoriesByType().map(cat => (
                    <motion.button
                      ...
                      className={`flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 transition-all ${formData.category === cat.value ? 'border-purple-500 bg-purple-50 text-purple-700 shadow-md' : 'border-gray-200 hover:border-purple-300'}`}
                    >
          ...
          {/* Colonne droite - Calendrier (inchangé mais modernisé) */}
