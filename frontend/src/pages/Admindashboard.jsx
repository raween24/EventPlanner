...
import {
  ...
  FileText, MapPin, Star, Filter, RefreshCw, ChevronUp,
  User, Building, Tag, DollarSign, Hash, TrendingUp
} from "lucide-react";
...
const Badge = ({ statut }) => {
  const map = {
    ...
    Other:          { bg: C.borderLight, color: C.t2,  label: "Autre" },
...
const UserAvatar = ({ user }) => {
  ...
  const [bg, col] = colors[init.charCodeAt(0) % colors.length];
  return (
    <div style={{ width: 36, height: 36, borderRadius: "50%", background: bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: col, flexShrink: 0 }}>
      {init}
    </div>
  );
};
    ...
    <>
      ...
      <aside style={{ width: isMobile ? "80%" : 256, maxWidth: isMobile ? 300 : "none", height: "100vh", background: C.sidebar, borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column", position: "fixed", left: 0, top: 0, zIndex: 200, transform: isMobile ? (isMobileOpen ? "translateX(0)" : "translateX(-100%)") : "none", transition: "transform .25s ease", overflowY: "auto" }}>
        <div style={{ padding: "20px 20px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <h1 style={{ fontSize: 18, fontWeight: 700, color: C.t1, fontFamily: "'Google Sans',sans-serif", letterSpacing: "-.3px" }}>Admin Panel</h1>
...
/* ─── TOPBAR ── */
const TopBar = ({ title, isMobile, searchValue, onSearchChange, showSearch, extra }) => {
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
        ...
        {showSearch && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, background: C.bg, border: `1px solid ${C.border}`, borderRadius: 24, padding: "8px 16px", width: isMobile ? 160 : 240 }}>
            ...
            <input value={searchValue} onChange={onSearchChange} placeholder="Rechercher..." style={{ border: "none", background: "transparent", fontSize: 14, color: C.t1, width: "100%", fontFamily: "Roboto,sans-serif" }} />
          </div>
        )}
...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [statsRes, monthlyRes, eventsRes, demandesRes, usersRes] = await Promise.all([
          ...
          axios.get(`${API}/location/all`, { headers: authHeaders() }).catch(() => ({ data: [] })),
          axios.get(`${API}/admin/users`, { headers: authHeaders() }),
        ]);
        setStats(statsRes.data);
        setMonthlyData(monthlyRes.data);
        const upcoming = eventsRes.data.filter(e => e.dateDebut).sort((a, b) => new Date(a.dateDebut) - new Date(b.dateDebut)).slice(0, 5);
        setEvents(upcoming);
        const sortedDemandes = [...(demandesRes.data || [])].sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0)).slice(0, 4);
        setDemandes(sortedDemandes);
        const sorted = [...usersRes.data].sort((a, b) => b._id.toString().localeCompare(a._id.toString())).slice(0, 6);
  ...
  const fillMonthly = (arr, total) =>
    Array.from({ length: 6 }, (_, i) => {
      ...
      return found?.count ?? Math.max(1, Math.round((total / 6) * (0.4 + (i / 5) * 0.6)));
    });
  ...
  const statusColor = s => ({
    ...
    confirmé:  { bg: "#d1fae5", color: "#065f46", label: "Confirmé" },
    refusé:    { bg: "#fee2e2", color: "#991b1b", label: "Refusé" },
    acceptée:  { bg: "#d1fae5", color: "#065f46", label: "Acceptée" },
    refusée:   { bg: "#fee2e2", color: "#991b1b", label: "Refusée" },
    annulé:    { bg: "#f1f5f9", color: "#475569", label: "Annulé" },
    "en cours":{ bg: "#e0f2fe", color: "#0c4a6e", label: "En cours" },
    terminé:   { bg: "#d1fae5", color: "#065f46", label: "Terminé" },
  }[s] || { bg: "#f1f5f9", color: "#475569", label: s || "—" });

  const avatarColors = [["#ede9fe","#5b21b6"],["#d1fae5","#065f46"],["#fef3c7","#92400e"],["#fee2e2","#991b1b"],["#e0f2fe","#0c4a6e"]];
  const initials = (u) => u?.organisateur ? `${u.organisateur.firstname?.[0]||""}${u.organisateur.lastname?.[0]||""}`.toUpperCase() : "??";
  ...
  const eventRate    = stats?.events ? Math.min(91, Math.round((stats.events / Math.max(stats.events, 1)) * 91)) : 0;
  const demandeRate  = 58;
  ...
  const notifications = [
    ...
    ...(stats?.pending > 0 ? [{ icon: "⚠️", bg: "#fef3c7", title: "Demandes en attente", body: `${stats.pending} prestataire(s) attend validation`, time: "Maintenant", unread: true }] : []),
  ].slice(0, 5);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
        {[
          ...
          { label: "Demandes",     val: stats?.pending,   sub: "en attente de traitement",                 stripe: `linear-gradient(90deg,${ORANGE},#fbbf24)`, iconBg: "#fef3c7", icon: <Clock size={20} color={ORANGE}/>, delta: "!", deltaColor: RED },
          ...
          <div key={i} className="adash-stat-card" style={{ position: "relative", overflow: "hidden", animationDelay: `${i * .08}s` }}>
            ...
            {loading
              ? <><Skel w="55%" h={30} r={4}/><Skel w="75%" h={12} r={4} style={{ marginTop: 8 }}/></>
              : <>
        ...
        <div style={cardStyle}>
          ...
          <div style={{ ...cardBody, paddingTop: 16 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, marginTop: 14 }}>
              {[
                { val: stats?.users ?? 0, lbl: "Total inscrits", col: PURPLE },
                { val: stats?.resources ?? 0, lbl: "Ressources", col: GREEN },
                { val: stats?.events ?? 0, lbl: "Événements", col: ORANGE },
              ].map(({ val, lbl, col }) => (
        ...
        <div style={cardStyle}>
          ...
          <div style={cardBody}>
            {[
              ...
              { lbl: "Événements confirmés",  val: eventRate,   col: GREEN,  grad: `linear-gradient(90deg,${GREEN},#34d399)` },
              { lbl: "Demandes traitées",     val: demandeRate, col: ORANGE, grad: `linear-gradient(90deg,${ORANGE},#fbbf24)` },
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
        <div style={cardStyle}>
          <div style={cardHead}>
            ...
            <span style={{ fontSize: 11, color: PURPLE, cursor: "pointer", fontWeight: 600 }}>Voir tous</span>
          </div>
                  ...
                  <div key={ev._id} style={rowItem(i === events.length - 1)}>
                    {d && (
                      <div style={{ minWidth: 38, textAlign: "center", background: "#f5f3ff", borderRadius: 8, padding: "4px 0", flexShrink: 0 }}>
                        ...
                        <div style={{ fontSize: 9, color: "#7c3aed" }}>{["Jan","Fév","Mar","Avr","Mai","Jun","Jui","Aoû","Sep","Oct","Nov","Déc"][d.getMonth()]}</div>
                      </div>
                    )}
        ...
        <div style={cardStyle}>
          <div style={cardHead}>
            <div style={{ fontSize: 14, fontWeight: 700, color: C.t1, fontFamily: "'Google Sans',sans-serif" }}>Demandes récentes</div>
            <span style={{ fontSize: 11, color: PURPLE, cursor: "pointer", fontWeight: 600 }}>Gérer</span>
          </div>
          <div style={{ padding: "4px 16px 12px" }}>
            {loading ? [1,2,3].map(i => <div key={i} style={{ padding: "10px 0" }}><Skel h={44} r={8}/></div>)
              : demandes.length === 0 ? <EmptyState icon={Clock} message="Aucune demande"/>
              : demandes.map((dm, i) => {
                const [bg, col] = avatarColors[i % avatarColors.length];
                const nom = dm.organisateur ? `${dm.organisateur.firstname || ""} ${dm.organisateur.lastname || ""}`.trim() : "Inconnu";
                const sc = statusColor(dm.status);
                return (
                  <div key={dm._id} style={rowItem(i === demandes.length - 1)}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 12.5, fontWeight: 600, color: C.t1, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{nom}</div>
                      <div style={{ fontSize: 10.5, color: C.t2, marginTop: 1 }}>{dm.resource?.name || "Ressource"}</div>
                    </div>
                    <span style={{ fontSize: 9.5, fontWeight: 700, padding: "2px 8px", borderRadius: 8, background: sc.bg, color: sc.color }}>{sc.label}</span>
                  </div>
                );
              })
            }
          ...
          <div style={cardHead}>
            <div style={{ fontSize: 14, fontWeight: 700, color: C.t1, fontFamily: "'Google Sans',sans-serif" }}>
              ...
              {notifications.filter(n => n.unread).length > 0 && (
                <span style={{ marginLeft: 8, fontSize: 10, fontWeight: 700, padding: "1px 6px", borderRadius: 8, background: "#fee2e2", color: "#991b1b" }}>{notifications.filter(n => n.unread).length} nouvelles</span>
              )}
            </div>
          </div>
          <div style={{ padding: "4px 16px 12px" }}>
                      ...
                      <tr key={u._id} className="adash-table-row">
                        ...
                        <TD>
                          <div style={{display:"flex",alignItems:"center",gap:6}}>
                            <button onClick={()=>setDetail(u)} title="Voir détails"
                              style={{width:32,height:32,borderRadius:8,border:"none",background:C.accentBg,color:C.accent,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>
                              <Eye size={15}/>
                            </button>
...
const RessourcesPage = () => {
  ...
  const loadComments = async (id) => {
    ...
    try {
      ...
      setComments(p => ({ ...p, [id]: data })); setExpanded(id);
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:20 }}>
        {types.map(t => (
          <button key={t} onClick={() => setFilter(t)} style={{ padding:"6px 16px", borderRadius:20, fontSize:13, fontWeight:500, cursor:"pointer", fontFamily:"inherit", background: filter===t ? C.accentBg : C.card, color: filter===t ? C.accent : C.t2, border: filter===t ? `1.5px solid ${C.accent}` : `1px solid ${C.border}`, transition:"all .15s" }}>
            {typeLabels[t]}
          </button>
        ))}
      </div>
                      ...
                      <tr key={r._id} className="adash-table-row">
                        ...
                        <TD>
                          <div style={{ display:"flex", gap:6 }}>
                            <button onClick={() => setDetail(r)} title="Détails" style={{ width:32, height:32, borderRadius:8, border:"none", background:C.accentBg, color:C.accent, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}><Info size={15}/></button>
                            <button onClick={() => handleDelete(r._id, r.name)} title="Supprimer" style={{ width:32, height:32, borderRadius:8, border:"none", background:C.redBg, color:C.red, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}><Trash2 size={16}/></button>
                          </div>
                        </TD>
                      </tr>
                      {expanded === r._id && (
                                        ...
                                        <div key={c._id} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:10, padding:"12px 16px" }}>
                                          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:8 }}>
                                            ...
                                            <div style={{ display:"flex", gap:2 }}>
                                              {[1,2,3,4,5].map(s => <span key={s} style={{ fontSize:16, color: s<=stars ? "#f59e0b" : "#e8eaed" }}>&#9733;</span>)}
                                            </div>
...
const EvenementsPage = () => {
  ...
  const load = useCallback(async () => {
    try {
      setLoad(true);
      const { data } = await axios.get(`${API}/admin/events`, { headers: authHeaders() });
      setEvents(data);
  ...
  const handleDelete = async (id, title) => {
    ...
    try {
      await axios.delete(`${API}/admin/events/${id}`, { headers: authHeaders() });
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      await axios.patch(`${API}/admin/events/${id}`, { status: newStatus }, { headers: authHeaders() });
      setEvents(p => p.map(e => e._id === id ? { ...e, status: newStatus } : e));
  ...
  const filtered = events.filter(e => {
    ...
    const mcat = categoryFilter === "tous" || e.category === categoryFilter;
    return ms && mst && mcat;
  });
  ...
  return (
    <div>
      ...
      <Toasts toasts={toast.toasts} remove={toast.remove}/>
      {detail && (
        <DetailModal data={detail} title={detail.title} onClose={() => setDetail(null)} fields={[
          { label: "Catégorie",     value: detail.category, badge: true },
          { label: "Type",          value: detail.type, badge: true },
          { label: "Statut",        value: detail.status, badge: true },
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8, background:C.card, border:`1px solid ${C.border}`, borderRadius:24, padding:"8px 16px", width:240 }}>
          <Search size={14} color={C.t3}/>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Rechercher un événement..." style={{ border:"none", background:"transparent", fontSize:13, color:C.t1, width:"100%", fontFamily:"inherit" }}/>
        </div>
      </div>
          ...
          : (
            <div style={{ overflowX:"auto" }}>
              <table style={{ width:"100%", borderCollapse:"collapse", minWidth:900 }}>
                <thead>
                  <tr style={{ background:C.bg, borderBottom:`1px solid ${C.border}` }}>
                    <TH>Événement</TH><TH>Catégorie</TH><TH>Type</TH><TH>Dates</TH><TH>Lieu</TH><TH>Participants</TH><TH>Statut</TH><TH>Actions</TH>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map(ev => (
                    <tr key={ev._id} className="adash-table-row">
                      <TD>
                        ...
                        <div style={{ fontSize:11, color:C.t3, marginTop:2 }}>{ev.ressources_utiliser?.length || 0} ressource(s)</div>
                      </TD>
                      ...
                      <TD>
                        <div style={{ display:"flex", alignItems:"center", gap:5, fontSize:13, color:C.t1 }}>
                          <Users size={13} color={C.t3}/>{ev.nombreParticipants || "—"}
                        </div>
                      </TD>
                      <TD>
                        <select
                          ...
                          style={{ padding:"4px 8px", borderRadius:8, border:`1px solid ${C.border}`, fontSize:12, color:C.t1, background:C.bg, cursor:"pointer", fontFamily:"inherit" }}>
                      ...
                      <TD>
                        <div style={{ display:"flex", gap:6 }}>
                          <button onClick={() => setDetail(ev)} title="Détails" style={{ width:32, height:32, borderRadius:8, border:"none", background:C.accentBg, color:C.accent, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}><Eye size={15}/></button>
                          <button onClick={() => handleDelete(ev._id, ev.title)} title="Supprimer" style={{ width:32, height:32, borderRadius:8, border:"none", background:C.redBg, color:C.red, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}><Trash2 size={16}/></button>
                        </div>
                      </TD>
                    </tr>
                  ))}
...
const DemandesPage = () => {
  ...
  const load = useCallback(async () => {
    try {
      setLoad(true);
      const { data } = await axios.get(`${API}/location/all`, { headers: authHeaders() });
      setDemandes(data);
    } catch { setDemandes([]); } finally { setLoad(false); }
  }, []);
  useEffect(() => { load(); }, [load]);

  const handleDelete = async (id) => {
    if (!await confirm({ type:"danger", title:"Supprimer cette demande ?", message:"Cette demande sera définitivement supprimée.", confirmLabel:"Supprimer" })) return;
    try {
      await axios.delete(`${API}/location/${id}`, { headers: authHeaders() });
      setDemandes(p => p.filter(d => d._id !== id));
  ...
  const handleStatusUpdate = async (id, status) => {
    try {
      await axios.patch(`${API}/location/${id}/status`, { status }, { headers: authHeaders() });
      setDemandes(p => p.map(d => d._id === id ? { ...d, status } : d));
      toast.success(`Demande ${status}`);
    } catch { toast.error("Erreur mise à jour"); }
  };

  const statuses = ["tous", "en attente", "acceptée", "refusée"];
  const statusCounts = { "en attente": 0, "acceptée": 0, "refusée": 0 };
  demandes.forEach(d => { if (statusCounts[d.status] !== undefined) statusCounts[d.status]++; });

  const filtered = demandes.filter(d => {
    const ms = search === "" || `${d.organisateur?.firstname} ${d.organisateur?.lastname} ${d.resource?.name} ${d.event?.title}`.toLowerCase().includes(search.toLowerCase());
    const mst = statusFilter === "tous" || d.status === statusFilter;
    return ms && mst;
  });
  ...
  return (
    <div>
      ...
      <Toasts toasts={toast.toasts} remove={toast.remove}/>
      {detail && (
        <DetailModal data={detail} title="Détail de la demande" onClose={() => setDetail(null)} fields={[
          { label: "Statut",       value: detail.status, badge: true },
          { label: "Organisateur", value: `${detail.organisateur?.firstname || ""} ${detail.organisateur?.lastname || ""}` },
          { label: "Email",        value: detail.organisateur?.email },
          { label: "Ressource",    value: detail.resource?.name },
          { label: "Événement",    value: detail.event?.title },
          { label: "Date début",   value: detail.dateDebut ? new Date(detail.dateDebut).toLocaleDateString("fr-FR") : "—" },
          { label: "Date fin",     value: detail.dateFin ? new Date(detail.dateFin).toLocaleDateString("fr-FR") : "—" },
          { label: "Créée le",     value: detail.createdAt ? new Date(detail.createdAt).toLocaleDateString("fr-FR") : "—" },
        ]} />
      )}

      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
        <div>
          <h2 style={{ fontSize:22, fontWeight:700, color:C.t1, fontFamily:"'Google Sans',sans-serif", margin:0 }}>Gestion des demandes</h2>
          <p style={{ fontSize:13, color:C.t2, margin:"4px 0 0" }}>{demandes.length} demandes au total</p>
        </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12, marginBottom:20 }}>
        {[
          { label:"En attente", count: statusCounts["en attente"], bg:"#fef3c7", color:"#92400e", border:"#fbbf24" },
          { label:"Acceptées",  count: statusCounts["acceptée"],   bg:C.greenBg, color:C.green,   border:"#34d399" },
          { label:"Refusées",   count: statusCounts["refusée"],    bg:C.redBg,   color:C.red,     border:"#f87171" },
        ].map(({ label, count, bg, color, border: brd }) => (
          <div key={label} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"16px 20px", borderLeft:`4px solid ${brd}` }}>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:20 }}>
        {statuses.map(s => (
          <button key={s} onClick={() => setStatus(s)} style={{ padding:"6px 16px", borderRadius:20, fontSize:13, fontWeight:500, cursor:"pointer", fontFamily:"inherit", background: statusFilter===s ? C.accentBg : C.card, color: statusFilter===s ? C.accent : C.t2, border: statusFilter===s ? `1.5px solid ${C.accent}` : `1px solid ${C.border}` }}>
            {s === "tous" ? `Toutes (${demandes.length})` : `${s.charAt(0).toUpperCase()}${s.slice(1)} (${statusCounts[s] || 0})`}
          </button>
        ))}
      </div>
      ...
      <div style={{ background:C.card, borderRadius:12, border:`1px solid ${C.border}`, overflow:"hidden" }}>
        {loading
          ? <div style={{padding:24}}>{[1,2,3,4].map(i=><div key={i} style={{marginBottom:12}}><Skel h={60} r={8}/></div>)}</div>
          : filtered.length === 0 ? <EmptyState icon={Clock} message="Aucune demande trouvée"/>
          : (
            <div style={{ overflowX:"auto" }}>
              <table style={{ width:"100%", borderCollapse:"collapse", minWidth:960 }}>
                <thead>
                  <tr style={{ background:C.bg, borderBottom:`1px solid ${C.border}` }}>
                    <TH>Organisateur</TH><TH>Ressource</TH><TH>Événement</TH><TH>Période</TH><TH>Statut</TH><TH>Créée le</TH><TH>Actions</TH>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map(d => (
                    <tr key={d._id} className="adash-table-row">
                      <TD>
                        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                          {d.organisateur && <UserAvatar user={d.organisateur}/>}
                          <div>
                            <div style={{ fontWeight:500, color:C.t1, fontSize:13 }}>{d.organisateur?.firstname} {d.organisateur?.lastname}</div>
                            <div style={{ fontSize:11, color:C.t3 }}>{d.organisateur?.email}</div>
                          </div>
                        </div>
                      </TD>
                      <TD>
                        <div style={{ fontWeight:500, color:C.t1, fontSize:13 }}>{d.resource?.name || "—"}</div>
                        {d.resource?.category && <Badge statut={d.resource.category}/>}
                      </TD>
                      <TD>
                        <div style={{ fontWeight:500, color:C.t1, fontSize:13 }}>{d.event?.title || "—"}</div>
                        {d.event?.category && <div style={{ fontSize:11, color:C.t3 }}>{d.event.category}</div>}
                      </TD>
                      <TD>
                        <div style={{ fontSize:12, color:C.t1 }}>{d.dateDebut ? new Date(d.dateDebut).toLocaleDateString("fr-FR") : "—"}</div>
                        <div style={{ fontSize:11, color:C.t3 }}>{d.dateFin ? `→ ${new Date(d.dateFin).toLocaleDateString("fr-FR")}` : ""}</div>
                      </TD>
                      <TD><Badge statut={d.status}/></TD>
                      <TD style={{ fontSize:12, color:C.t2 }}>{d.createdAt ? new Date(d.createdAt).toLocaleDateString("fr-FR") : "—"}</TD>
                      <TD>
                        <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                          <button onClick={() => setDetail(d)} title="Détails" style={{ width:32, height:32, borderRadius:8, border:"none", background:C.accentBg, color:C.accent, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}><Eye size={15}/></button>
                          {d.status === "en attente" && <>
                            <button onClick={() => handleStatusUpdate(d._id, "acceptée")} title="Accepter" style={{ width:32, height:32, borderRadius:8, border:"none", background:C.greenBg, color:C.green, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}><CheckCircle size={16}/></button>
                            <button onClick={() => handleStatusUpdate(d._id, "refusée")} title="Refuser" style={{ width:32, height:32, borderRadius:8, border:"none", background:C.redBg, color:C.red, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}><XCircle size={16}/></button>
                          </>}
                          <button onClick={() => handleDelete(d._id)} title="Supprimer" style={{ width:32, height:32, borderRadius:8, border:"none", background:C.redBg, color:C.red, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}><Trash2 size={16}/></button>
                        </div>
                      </TD>
                    </tr>
                  ))}
...
const DocumentsPage = () => {
  ...
  const load = useCallback(async () => {
    try {
      setLoad(true);
      const { data } = await axios.get(`${API}/admin/documents`, { headers: authHeaders() });
      setDocuments(Array.isArray(data) ? data : data.data || []);
    } catch { setDocuments([]); } finally { setLoad(false); }
  }, []);
  useEffect(() => { load(); }, [load]);
  ...
  const docTypes = ["tous", "CIN", "Passport", "Contract", "Other"];
  const typeCounts = { CIN: 0, Passport: 0, Contract: 0, Other: 0 };
  ...
  const docIcon = (type) => {
    const icons = { CIN: "🪪", Passport: "📘", Contract: "📄", Other: "📎" };
    return icons[type] || "📁";
  };
    ...
    <div>
      ...
      {detail && (
        <DetailModal data={detail} title={detail.title} onClose={() => setDetail(null)} fields={[
          ...
          { label: "URL fichier",value: detail.fileUrl },
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8, background:C.card, border:`1px solid ${C.border}`, borderRadius:24, padding:"8px 16px", width:240 }}>
          <Search size={14} color={C.t3}/>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Rechercher un document..." style={{ border:"none", background:"transparent", fontSize:13, color:C.t1, width:"100%", fontFamily:"inherit" }}/>
        </div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:20 }}>
        {[
          { type:"CIN",      label:"CIN",       col:C.accent,  bg:C.accentBg },
          { type:"Passport", label:"Passeports", col:C.purple,  bg:C.purpleBg },
          { type:"Contract", label:"Contrats",   col:C.green,   bg:C.greenBg },
          { type:"Other",    label:"Autres",     col:C.orange,  bg:C.orangeBg },
        ].map(({ type, label, col, bg }) => (
          <div key={type} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"16px 20px", display:"flex", alignItems:"center", gap:12 }}>
      ...
      <div style={{ background:C.card, borderRadius:12, border:`1px solid ${C.border}`, overflow:"hidden" }}>
        {loading
          ? <div style={{padding:24}}>{[1,2,3,4].map(i=><div key={i} style={{marginBottom:12}}><Skel h={56} r={8}/></div>)}</div>
          ...
          : (
            <div style={{ overflowX:"auto" }}>
              <table style={{ width:"100%", borderCollapse:"collapse", minWidth:760 }}>
                <thead>
                  <tr style={{ background:C.bg, borderBottom:`1px solid ${C.border}` }}>
                    <TH>Document</TH><TH>Type</TH><TH>Utilisateur</TH><TH>Ressources liées</TH><TH>Créé le</TH><TH>Actions</TH>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map(d => (
                    <tr key={d._id} className="adash-table-row">
                      <TD>
                        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                          <div style={{ width:38, height:38, borderRadius:10, background: d.documentType === "CIN" ? C.accentBg : d.documentType === "Passport" ? C.purpleBg : d.documentType === "Contract" ? C.greenBg : C.orangeBg, display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, flexShrink:0 }}>
                            {docIcon(d.documentType)}
                          </div>
                          <div>
                            <div style={{ fontWeight:500, color:C.t1, fontSize:13 }}>{d.title}</div>
                      ...
                      <TD>
                        <span style={{ fontSize:12, color:C.t2 }}>{d.resource?.length ? `${d.resource.length} ressource(s)` : "Aucune"}</span>
                      </TD>
                      <TD style={{ fontSize:12, color:C.t2 }}>{d.createdAt ? new Date(d.createdAt).toLocaleDateString("fr-FR") : "—"}</TD>
                      <TD>
                        <div style={{ display:"flex", gap:6 }}>
                          ...
                          <button onClick={() => handleDelete(d._id, d.title)} title="Supprimer" style={{ width:32, height:32, borderRadius:8, border:"none", background:C.redBg, color:C.red, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}><Trash2 size={16}/></button>
                        </div>
                      </TD>
                    </tr>
                  ))}
