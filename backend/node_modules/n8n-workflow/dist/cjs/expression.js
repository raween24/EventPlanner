...
(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "@n8n/errors", "@n8n/expression-runtime", "luxon", "./errors", "./errors/expression-extension.error", "./errors/expression.error", "./expression-evaluator-proxy", "./expression-sandboxing", "./expressions/expression-helpers", "./extensions", "./extensions/expression-extension", "./extensions/extended-functions"], factory);
    }
    ...
    class Expression {
        ...
        static expressionEngine = (() => {
            ...
            if (env === 'vm' || env === 'current')
                return env;
            if (env) {
                console.warn(`Unknown N8N_EXPRESSION_ENGINE="${env}", falling back to "current". Valid values: current, vm`);
            }
            return 'current';
        })();
        static vmEvaluator;
         ...
         */
        static async initializeVmEvaluator() {
            if (this.expressionEngine !== 'vm' || IS_FRONTEND)
                return;
            if (!this.vmEvaluator) {
         ...
         */
        static async disposeVmEvaluator() {
            if (this.vmEvaluator) {
        ...
        renderExpression(expression, data) {
            ...
            if (Expression.expressionEngine === 'vm' && !IS_FRONTEND) {
                if (!Expression.vmEvaluator) {
                    throw new errors_2.UnexpectedError('N8N_EXPRESSION_ENGINE=vm is enabled but VM evaluator is not initialized. Call Expression.initializeVmEvaluator() during application startup.');
                }
                try {
                    ...
                    if (isSyntaxError(error))
                        throw new expression_error_1.ExpressionError('invalid syntax');
                    throw error;
                }
            }
