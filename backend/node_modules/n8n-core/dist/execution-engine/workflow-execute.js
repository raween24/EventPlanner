...
const convert_binary_data_1 = require("../utils/convert-binary-data");
class WorkflowExecute {
    constructor(additionalData, mode, runExecutionData = (0, n8n_workflow_1.createRunExecutionData)(), storedAt = 'db') {
    ...
    checkReadyForExecution(workflow, inputData = {}) {
        ...
        for (const nodeName of checkNodes) {
            ...
            if (!node && nodeName === constants_1.TOOL_EXECUTOR_NODE_NAME) {
                continue;
            }
    ...
    async runNode(workflow, executionData, runExecutionData, runIndex, additionalData, mode, abortSignal, subNodeExecutionResults) {
        ...
        if (nodeType.supplyData) {
            throw new n8n_workflow_1.ApplicationError(`The node "${node.type}" has a "supplyData" method but no "execute" method.`);
        }
                    ...
                    for (let tryIndex = 0; tryIndex < maxTries; tryIndex++) {
                        try {
                            ...
                            if (pinData && !executionNode.disabled && pinData[executionNode.name] !== undefined) {
                                ...
                                runNodeData = await (0, convert_binary_data_1.convertBinaryData)(workflow.id, this.additionalData.executionId, runNodeData, workflow.settings.binaryMode);
    ...
    assignPairedItems(nodeSuccessData, executionData) {
        if (nodeSuccessData?.length) {
            ...
            const isSingleOutput = nodeSuccessData.length === 1 &&
