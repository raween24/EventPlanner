...
const getBeforeRedirectFn = (agentOptions, axiosConfig, proxyConfig, sendCredentialsOnCrossOriginRedirect, ssrfBridge) => (redirectedRequest) => {
    if (ssrfBridge) {
...
async function parseRequestObject(requestObject, ssrfBridge) {
    ...
    axiosConfig.beforeRedirect = getBeforeRedirectFn(agentOptions, axiosConfig, requestObject.proxy, requestObject.sendCredentialsOnCrossOriginRedirect ?? true, ssrfBridge);
...
function convertN8nRequestToAxios(n8nRequest, ssrfBridge) {
    ...
    axiosRequest.beforeRedirect = getBeforeRedirectFn(agentOptions, axiosRequest, n8nRequest.proxy, n8nRequest.sendCredentialsOnCrossOriginRedirect ?? true, ssrfBridge);
...
const getRequestHelperFunctions = (workflow, node, additionalData, runExecutionData = null, connectionInputData = []) => {
    ...
    return {
        httpRequest: async (requestOptions) => await httpRequest(requestOptions, additionalData.ssrfBridge),
