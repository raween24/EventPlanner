...
const utils_1 = require("../Form/utils/utils");
...
class Wait extends Webhook_node_1.Webhook {
        ...
        properties: [
            {
                ...
                type: 'options',
                builderHint: {
                    message: 'For user approval workflows, consider using nodes with operation: "sendAndWait" (e.g., email, Slack) instead of Wait node. If using "webhook", the URL will be generated at runtime and can be referenced with {{ $execution.resumeUrl }}.',
                },
                options: [
                    {
                        name: 'After Time Interval',
            ...
            },
            {
                ...
                options: [common_descriptions_1.appendAttributionToForm, common_descriptions_1.respondWithOptions, webhookSuffix],
            },
            {
                displayName: 'Options',
                name: 'options',
                ...
                options: [common_descriptions_1.appendAttributionToForm, webhookSuffix],
            },
        ],
    };
    async webhook(context) {
        ...
        if (resume === 'form')
            return await (0, utils_1.formWebhook)(context, this.authPropertyName);
    ...
    async execute(context) {
        ...
        if (['webhook', 'form'].includes(resume)) {
            ...
            if (resume === 'form') {
                ...
                hasFormTrigger = parentNodes.some((node) => node.type === n8n_workflow_1.FORM_TRIGGER_NODE_TYPE);
            }
        ...
        if (resume === 'timeInterval') {
            ...
            if (!(0, validation_1.validateWaitUnit)(unit)) {
                throw new n8n_workflow_1.NodeOperationError(context.getNode(), "Invalid wait unit. Valid units are 'seconds', 'minutes', 'hours', or 'days'.");
            }
            let waitAmount = context.getNodeParameter('amount', 0);
            if (!(0, validation_1.validateWaitAmount)(waitAmount)) {
                throw new n8n_workflow_1.NodeOperationError(context.getNode(), 'Invalid wait amount. Please enter a number that is 0 or greater.');
            }
            if (unit === 'minutes') {
                waitAmount *= 60;
            }
            ...
            try {
                ...
                const parsedDateTime = (0, n8n_workflow_1.tryToParseDateTime)(dateTimeStrRaw, context.getTimezone());
                waitTill = parsedDateTime.toUTC().toJSDate();
            }
            catch (e) {
                throw new n8n_workflow_1.NodeOperationError(context.getNode(), 'Cannot put execution to wait because `dateTime` parameter is not a valid date. Please pick a specific date and time to wait until.');
            }
        }
