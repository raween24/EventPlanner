            ...
            send: {
                ...
                preSend: [
                    async function (requestOptions) {
                        const userPrincipalName = this.getNodeParameter('userPrincipalName');
                        if (!/^[A-Za-z0-9'._\-!#^~@]+$/.test(userPrincipalName)) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), "Only the following characters are allowed for 'User Principal Name': A-Z, a-z, 0-9, ' . - _ ! # ^ ~");
                        }
            ...
            },
            {
                ...
                routing: {
                    send: {
                        preSend: [
                            async function (requestOptions) {
                                const companyName = this.getNodeParameter('additionalFields.companyName');
                                if (companyName?.length > 64) {
                                    throw new n8n_workflow_1.NodeOperationError(this.getNode(), "'Company Name' should have a maximum length of 64");
                                }
                                return requestOptions;
                            },
                        ],
            ...
            },
            {
                ...
                routing: {
                    send: {
                        preSend: [
                            async function (requestOptions) {
                                const employeeId = this.getNodeParameter('additionalFields.employeeId');
                                if (employeeId?.length > 16) {
                                    throw new n8n_workflow_1.NodeOperationError(this.getNode(), "'Employee ID' should have a maximum length of 16");
                                }
                                return requestOptions;
                            },
                        ],
                    ...
                    async function (items, _response) {
                        for (const item of items) {
                            ...
                            if (Object.keys(fields).length) {
                                ...
                                if (body.birthday) {
                                    body.birthday = body.birthday.toUTC().toISO();
                                }
                                if (body.businessPhones) {
                                    body.businessPhones = [body.businessPhones];
                                }
                                if (body.employeeHireDate) {
                                    body.employeeHireDate = body.employeeHireDate.toUTC().toISO();
                                }
                                if (body.employeeLeaveDateTime) {
                                    body.employeeLeaveDateTime = body.employeeLeaveDateTime
                                        .toUTC()
                                        .toISO();
                                }
                                if (body.employeeOrgData) {
                                    body.employeeOrgData = body.employeeOrgData.employeeOrgValues;
                                }
                                if (body.passwordPolicies) {
                                    body.passwordPolicies = body.passwordPolicies.join(',');
                                }
                                ...
                                if (body.forceChangePassword) {
                                    if (body.forceChangePassword === 'forceChangePasswordNextSignIn') {
                                        ...
                                        body.passwordProfile ??= {};
                                        body.passwordProfile.forceChangePasswordNextSignInWithMfa =
                                            true;
                                    }
                                ...
                                try {
                                    if (Object.keys(separateBody).length) {
                                        await GenericFunctions_1.microsoftApiRequest.call(this, 'PATCH', `/users/${userId}`, separateBody);
                                        (0, merge_1.default)(item.json, separateBody);
                                    }
                                    if (Object.keys(body).length) {
                                        await GenericFunctions_1.microsoftApiRequest.call(this, 'PATCH', `/users/${userId}`, body);
                                        (0, merge_1.default)(item.json, body);
                                    }
                                }
                                catch (error) {
                                    try {
                                        await GenericFunctions_1.microsoftApiRequest.call(this, 'DELETE', `/users/${userId}`);
                                    }
                                    catch { }
                                    throw error;
                                }
            ...
            },
            {
                ...
                routing: {
                    send: {
                        ...
                        preSend: [
                            async function (requestOptions) {
                                const companyName = this.getNodeParameter('updateFields.companyName');
                                if (companyName?.length > 64) {
                                    throw new n8n_workflow_1.NodeOperationError(this.getNode(), "'Company Name' should have a maximum length of 64");
                                }
                                return requestOptions;
            ...
            },
            {
                ...
                routing: {
                    send: {
                        ...
                        preSend: [
                            async function (requestOptions) {
                                const employeeId = this.getNodeParameter('updateFields.employeeId');
                                if (employeeId?.length > 16) {
                                    throw new n8n_workflow_1.NodeOperationError(this.getNode(), "'Employee ID' should have a maximum length of 16");
                                }
                                return requestOptions;
            ...
            },
            {
                ...
                routing: {
                    send: {
                        preSend: [
                            async function (requestOptions) {
                                const forceChangePassword = this.getNodeParameter('updateFields.forceChangePassword');
                                if (forceChangePassword === 'forceChangePasswordNextSignIn') {
                                    requestOptions.body.passwordProfile ??= {};
                                    requestOptions.body.passwordProfile.forceChangePasswordNextSignIn = true;
                                }
                                else if (forceChangePassword === 'forceChangePasswordNextSignInWithMfa') {
                                    requestOptions.body.passwordProfile.forceChangePasswordNextSignInWithMfa = true;
                                }
                                return requestOptions;
                            },
                        ],
            ...
            },
            {
                ...
                routing: {
                    send: {
                        ...
                        preSend: [
                            async function (requestOptions) {
                                const userPrincipalName = this.getNodeParameter('updateFields.userPrincipalName');
                                if (!/^[A-Za-z0-9'._\-!#^~@]+$/.test(userPrincipalName)) {
                                    throw new n8n_workflow_1.NodeOperationError(this.getNode(), "Only the following characters are allowed for 'User Principal Name': A-Z, a-z, 0-9, ' . - _ ! # ^ ~");
                                }
                                return requestOptions;
        ...
        routing: {
            output: {
                postReceive: [
                    async function (items, _response) {
                        for (const item of items) {
                            const userId = this.getNodeParameter('user.value', item.index);
