...
function validateAttachmentUrl(url, allowedDomain) {
    let attachmentUrl;
    let allowedUrl;
    try {
    ...
    const matchesDomain = attachmentHost === allowedHost || attachmentHost.endsWith(`.${allowedHost}`);
    if (matchesDomain)
        return;
    const isKoboDomain = KOBO_DOMAINS.some((domain) => allowedHost === domain || allowedHost.endsWith(`.${domain}`));
    if (isKoboDomain) {
        const isFromKoboDomain = KOBO_DOMAINS.some((domain) => attachmentHost === domain || attachmentHost.endsWith(`.${domain}`));
        if (isFromKoboDomain)
            return;
    }
    throw new Error(`Attachment URL domain (${attachmentHost}) does not match allowed domain (${allowedHost})`);
}
async function downloadAttachments(submission, options) {
    ...
    const credentials = await this.getCredentials('koBoToolboxApi');
    const koboToolboxUrl = credentials.URL;
