    ...
    constructor(baseDescription) {
        this.description = {
            ...
            properties: [
                ...
                ], undefined, undefined, { extraOptions: [MessageDescription_1.replyToMessageField] }).filter((p) => p.name !== 'subject'),
    ...
    methods = {
        listSearch: {
            async getChannels(filter, paginationToken) {
                ...
                const { data: channels, cursor } = await (0, GenericFunctions_1.slackApiRequestAllItemsWithRateLimit)(this, 'channels', 'GET', '/conversations.list', {}, qs, {
                    onFail: 'stop',
                    maxRetries: 2,
            ...
            try {
                ...
                if (resource === 'userGroup') {
                    ...
                    if (operation === 'updateUsers') {
                        ...
                        const currentGroupData = await GenericFunctions_1.slackApiRequest.call(this, 'GET', '/usergroups.list', {}, { ...qs, include_users: true, usergroup: userGroupId });
                        ...
                        Object.assign(body, options);
                        responseData = await GenericFunctions_1.slackApiRequest.call(this, 'POST', '/usergroups.users.update', body, qs);
                        responseData = responseData.usergroup;
                    }
                    //https://api.slack.com/methods/usergroups.list
                    if (operation === 'getUsers') {
                        ...
                        const groupData = await GenericFunctions_1.slackApiRequest.call(this, 'GET', '/usergroups.list', {}, { ...qs, include_users: true, usergroup: userGroupId });
                        // Find the specific user group
                        const userGroup = groupData.usergroups.find((group) => group.id === userGroupId);
                        if (!userGroup) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), `User group with ID "${userGroupId}" not found`);
                        }
                        ...
                        if (resolveData && userIds.length > 0) {
                            ...
                            for (const userId of userIds) {
                                const { user } = await GenericFunctions_1.slackApiRequest.call(this, 'GET', '/users.info', {}, { user: userId });
                                users.push(user);
                            }
                            responseData = users;
                        }
