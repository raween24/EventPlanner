...
class RabbitMQTrigger {
    ...
    async trigger() {
        ...
        if (this.getMode() === 'manual') {
            const manualTriggerFunction = async () => {
                ...
                const processMessage = async (message) => {
                    if (message !== null) {
                        void GenericFunctions_1.handleMessage.call(this, message, channel, messageTracker, acknowledgeMode, options);
                    }
                    else {
        ...
        if (isNaN(parallelMessages) || parallelMessages === 0 || parallelMessages < -1) {
            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Parallel message processing limit must be a number greater than zero (or -1 for no limit)');
        }
