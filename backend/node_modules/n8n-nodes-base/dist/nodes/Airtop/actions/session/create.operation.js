...
const countryOptions = countries_1.COUNTRIES.map(({ name, value }) => ({ name, value }));
exports.description = [
    {
...
async function execute(index) {
    ...
    const { sessionId, data } = await GenericFunctions_1.createSession.call(this, body);
    if (saveProfileOnTermination) {
        await transport_1.apiRequest.call(this, 'PUT', `/sessions/${sessionId}/save-profile-on-termination/${profileName}`);
    }
