...
function normalizeOutBinds(outBinds, stmtBatching, outputColumns) {
    ...
    if (!Array.isArray(outBinds)) {
        ...
        const row = {};
        for (const [key, val] of Object.entries(outBinds)) {
...
function isSerializedBuffer(val) {
            ...
            switch (type) {
                ...
                case 'blob':
                    ...
                    if (isSerializedBuffer(bindVal)) {
                        bindVal = Buffer.from(bindVal.data);
                        break;
                    }
                    throw new n8n_workflow_1.UserError('BLOB data must be a valid Buffer or \'{ type: "Buffer", data: [...] }\'');
                case 'date': {
