import type { INodeProperties, INodePropertyOptions } from 'n8n-workflow';
export declare const formElementTypes: INodePropertyOptions[];
export declare const placeholder: string;
export declare const webhookPath: INodeProperties;
export declare const formTitle: INodeProperties;
export declare const formDescription: INodeProperties;
export declare const ipAllowlist: INodeProperties;
export declare const formFields: INodeProperties;
export declare const formFieldsDynamic: INodeProperties;
export declare const formRespondMode: INodeProperties;
export declare const formTriggerPanel: {
    header: string;
    executionsHelp: {
        inactive: string;
        active: string;
    };
    activationHint: {
        active: string;
        inactive: string;
    };
};
export declare const respondWithOptions: INodeProperties;
export declare const appendAttributionToForm: INodeProperties;
//# sourceMappingURL=common.descriptions.d.ts.map