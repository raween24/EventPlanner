...
class CryptoV2 {
    ...
    async execute() {
        ...
        if (action === 'hmac' || action === 'sign') {
            const credentials = await this.getCredentials('crypto');
            if (action === 'hmac') {
                if (!credentials.hmacSecret) {
                    throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'No HMAC secret set in credentials. Please add an HMAC secret to your Crypto credentials.');
                }
                hmacSecret = credentials.hmacSecret;
            }
            if (action === 'sign') {
                if (!credentials.signPrivateKey) {
                    throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'No private key set in credentials. Please add a private key to your Crypto credentials.');
                }
                signPrivateKey = (0, utilities_1.formatPrivateKey)(credentials.signPrivateKey);
            }
        }
        let item;
        for (let i = 0; i < length; i++) {
            try {
                ...
                if (action === 'hash' || action === 'hmac') {
                    ...
                    const hashOrHmac = action === 'hash' ? (0, crypto_1.createHash)(type) : (0, crypto_1.createHmac)(type, hmacSecret);
...
exports.CryptoV2 = CryptoV2;
//# sourceMappingURL=CryptoV2.node.js.map