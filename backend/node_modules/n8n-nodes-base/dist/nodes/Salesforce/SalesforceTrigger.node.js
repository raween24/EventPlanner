...
class SalesforceTrigger {
    ...
    async poll() {
        ...
        const endDate = luxon_1.DateTime.now().toISO();
        if (!workflowData.processedIds) {
            workflowData.processedIds = [];
        }
        const processedIds = workflowData.processedIds;
        try {
            const pollStartDate = (0, GenericFunctions_1.getPollStartDate)(workflowData.lastTimeChecked);
            ...
            const { newItems, updatedProcessedIds } = (0, GenericFunctions_1.filterAndManageProcessedItems)(responseData, processedIds);
