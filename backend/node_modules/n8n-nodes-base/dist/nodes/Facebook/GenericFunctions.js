...
async function facebookApiRequest(method, resource, body = {}, qs = {}, uri, _option = {}) {
    let credentials;
    ...
    if (credentials.appSecret) {
        ...
        qs.appsecret_proof = (0, crypto_1.createHmac)('sha256', credentials.appSecret)
            .update(`${credentials.accessToken}|${appsecretTime}`)
            .digest('hex');
...
function getFields(object) {
    ...
    return [{ name: '*', value: '*' }].concat(data[object] || []).map((fieldObject) => ({
        ...fieldObject,
        name: fieldObject.value !== '*' ? (0, change_case_1.capitalCase)(fieldObject.value) : fieldObject.value,
    }));
}
