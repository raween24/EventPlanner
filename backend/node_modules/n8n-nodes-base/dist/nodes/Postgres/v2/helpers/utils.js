...
exports.runQueriesAndHandleErrors = exports.getWhereClauses = exports.isWhereClause = exports.convertArraysToPostgresFormat = exports.configureTableSchemaUpdater = void 0;
...
function prepareErrorItem(error, index) {
    return {
        json: { error: { ...error } },
...
function addWhereClauses(_node, _itemIndex, query, clauses, replacements, combineConditions) {
    if (clauses.length === 0)
...
const convertArraysToPostgresFormat = (data, schema, node, itemIndex = 0) => {
    const newData = (0, n8n_workflow_1.deepCopy)(data);
...
const conditionSet = new Set(common_descriptions_1.operatorOptions.map((option) => option.value)).add('=');
const isWhereClause = (clause) => {
...
exports.isWhereClause = isWhereClause;
const getWhereClauses = (ctx, itemIndex) => {
    ...
    const someInvalid = whereClausesValues.some((clause) => !(0, exports.isWhereClause)(clause));
    if (someInvalid) {
        throw new n8n_workflow_1.NodeOperationError(ctx.getNode(), 'Invalid where clause', {
            itemIndex,
        });
    }
...
const runQueriesAndHandleErrors = async (runQueries, queries, nodeOptions, errorItemsMap) => {
...
exports.runQueriesAndHandleErrors = runQueriesAndHandleErrors;
