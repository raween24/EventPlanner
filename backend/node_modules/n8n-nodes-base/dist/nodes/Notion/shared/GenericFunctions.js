...
exports.prepareNotionError = void 0;
...
async function notionApiRequestGetBlockChildrens(blocks, responseData = [], limit) {
    ...
    for (const block of blocks) {
        ...
        if (block.type === 'child_page' || block.type === 'unsupported')
            continue;
...
function getPropertyTitle(properties) {
    return (Object.values(properties).filter((property) => property.type === 'title')[0].title[0]
        ?.plain_text || '');
}
...
function extractBlockId(nodeVersion, itemIndex) {
    ...
    if (nodeVersion < 2.2) {
        ...
        if (blockIdRLCData.mode === 'id') {
            ...
            if (match === null) {
                const pageRegex = new RegExp(constants_1.blockUrlExtractionRegexp);
