...
class Xml {
    ...
    async execute() {
        ...
        if (typeof options.attrkey === 'string' && forbiddenKeys.includes(options.attrkey)) {
            throw new n8n_workflow_1.NodeOperationError(this.getNode(), `The "Attribute Key" option value "${options.attrkey}" is not allowed`);
        }
        if (typeof options.charkey === 'string' && forbiddenKeys.includes(options.charkey)) {
            throw new n8n_workflow_1.NodeOperationError(this.getNode(), `The "Character Key" option value "${options.charkey}" is not allowed`);
        }
        let item;
        const returnData = [];
