                ...
                options: [
                    {
                        ...
                        builderHint: {
                            message: 'You can add multiple intervals to trigger at different times. Use "Custom (Cron)" for more specific scheduling patterns.',
                        },
                        values: [
                            {
                                displayName: 'Trigger Interval',
    ...
    async trigger() {
        ...
        if (version >= 1.3) {
            for (let i = 0; i < intervals.length; i++) {
                (0, GenericFunctions_1.validateInterval)(this.getNode(), i, intervals[i]);
            }
        }
        const executeTrigger = (recurrence) => {
            const shouldTrigger = (0, GenericFunctions_1.recurrenceCheck)(recurrence, staticData.recurrenceRules, timezone);
            if (!shouldTrigger)
                return;
        ...
        const rules = intervals.map((interval, i) => ({
            interval,
            cronExpression: (0, GenericFunctions_1.toCronExpression)(interval),
            recurrence: (0, GenericFunctions_1.intervalToRecurrence)(interval, i),
        }));
        if (this.getMode() !== 'manual') {
            ...
            const manualTriggerFunction = async () => {
                ...
                if (interval.field === 'cronExpression') {
                    try {
                        (0, cron_1.sendAt)(cronExpression);
                    }
                    catch (error) {
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Invalid cron expression', {
                            description: 'More information on how to build them at https://crontab.guru/',
