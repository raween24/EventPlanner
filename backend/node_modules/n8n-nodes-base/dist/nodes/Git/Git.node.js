        ...
        for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
            try {
                ...
                if (isFilePathBlocked) {
                    throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Access to the repository path is not allowed');
                }
                const options = this.getNodeParameter('options', itemIndex, {});
                if (operation === 'clone') {
                    // Create repository folder if it does not exist
                    try {
                        await (0, promises_1.access)(resolvedRepositoryPath);
                    }
                    catch (error) {
                        await (0, promises_1.mkdir)(resolvedRepositoryPath);
                    }
                }
                const gitConfig = [];
                ...
                if (operation === 'add') {
                    ...
                    let append = false;
                    if (!enableGitNodeAllConfigKeys && !descriptions_1.ALLOWED_CONFIG_KEYS.includes(key)) {
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), `The provided git config key '${key}' is not allowed`);
                    }
                    ...
                    if (options.reference !== undefined && options.reference !== '') {
                        (0, n8n_workflow_1.assertParamIsString)('reference', options.reference, this.getNode());
                        (0, GenericFunctions_1.validateGitReference)(options.reference, this.getNode());
                    ...
                    returnItems.push.apply(returnItems, this.helpers.returnJsonArray(entries).map((item) => {
                        return {
                            ...item,
                            pairedItem: { item: itemIndex },
                        };
                    ...
                    // ----------------------------------
                    const config = await git.listConfig();
                    const data = (0, GenericFunctions_1.mapGitConfigList)(config);
