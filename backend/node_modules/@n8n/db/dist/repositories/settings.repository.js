...
let SettingsRepository = class SettingsRepository extends typeorm_1.Repository {
    constructor(dataSource) {
        super(entities_1.Settings, dataSource.manager);
    }
    async findByKey(key, em) {
        const manager = em ?? this.manager;
        return await manager.findOneBy(entities_1.Settings, { key });
    }
    async findByKeys(keys) {
        return await this.findBy({ key: (0, typeorm_1.In)(keys) });
    }
    async findByKeyPrefix(prefix) {
        return await this.findBy({ key: (0, typeorm_1.Like)(`${prefix}%`) });
    }
};
