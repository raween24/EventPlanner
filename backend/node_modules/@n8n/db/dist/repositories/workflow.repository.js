...
let WorkflowRepository = class WorkflowRepository extends typeorm_1.Repository {
    constructor(dataSource, globalConfig, folderRepository, workflowHistoryRepository) {
    ...
    async getWorkflowsAndFoldersWithCountWithSharingSubquery(user, sharingOptions, options = {}) {
        if (options.filter?.parentFolderId &&
            typeof options.filter?.parentFolderId === 'string' &&
            options.filter.parentFolderId !== n8n_workflow_1.PROJECT_ROOT &&
            typeof options.filter?.projectId === 'string' &&
    ...
    async getWorkflowsAndFoldersUnionWithSharingSubquery(user, sharingOptions, options = {}) {
        const { baseQuery, sortByColumn, sortByDirection } = this.buildBaseUnionQueryWithSharingSubquery(user, sharingOptions, options);
        const query = this.buildUnionQuery(baseQuery, {
            sortByColumn,
            sortByDirection,
            pagination: {
    ...
    async getWorkflowsAndFoldersCountWithSharingSubquery(user, sharingOptions, options = {}) {
        const { skip, take, ...baseQueryParameters } = options;
        const { baseQuery } = this.buildBaseUnionQueryWithSharingSubquery(user, sharingOptions, baseQueryParameters);
        const response = await baseQuery
            .select('COUNT(*)', 'count')
            .from('RESULT_QUERY', 'RESULT')
            .getRawOne();
    ...
    buildBaseUnionQueryWithSharingSubquery(user, sharingOptions, options = {}) {
        const commonFields = {
            updatedAt: true,
            createdAt: true,
            id: true,
        ...
        const foldersQuery = this.folderRepository
            ...
            .addSelect('NULL', 'description')
            .addSelect("'folder'", 'resource');
        const workflowsQuery = this.getManyQueryWithSharingSubquery(user, sharingOptions, workflowQueryParameters).addSelect("'workflow'", 'resource');
        const qb = this.manager.createQueryBuilder();
        return {
            baseQuery: qb
                .createQueryBuilder()
    ...
    buildSharedWorkflowIdsSubquery(user, sharingOptions) {
        const { projectRoles, workflowRoles, isPersonalProject, personalProjectOwnerId, onlySharedWithMe, projectId, } = sharingOptions;
        const subquery = this.manager
            .createQueryBuilder()
            .select('sw.workflowId')
            .from(entities_1.SharedWorkflow, 'sw');
        if (isPersonalProject) {
            const ownerUserId = personalProjectOwnerId ?? user.id;
            subquery
                .innerJoin(entities_1.Project, 'p', 'sw.projectId = p.id')
        ...
        else if ((0, permissions_1.hasGlobalScope)(user, 'workflow:read')) {
            if (projectId && typeof projectId === 'string' && projectId !== '') {
    ...
    applyAvailableInMCPFilter(qb, filter) {
        if (typeof filter?.availableInMCP === 'boolean') {
            ...
            if (filter.availableInMCP) {
                ...
                if (dbType === 'postgresdb') {
                    qb.andWhere("(workflow.settings ->> 'availableInMCP' = :availableInMCP OR workflow.settings ->> 'availableInMCP' IS NULL)", { availableInMCP: 'false' });
                }
                else if (dbType === 'sqlite') {
                    qb.andWhere("(JSON_EXTRACT(workflow.settings, '$.availableInMCP') = :availableInMCP OR JSON_EXTRACT(workflow.settings, '$.availableInMCP') IS NULL)", { availableInMCP: 0 });
                }
            }
        }
    }
    ...
    applySelect(qb, select) {
        ...
        const regularFields = Object.entries(select).filter(([field]) => !['ownedBy', 'tags', 'parentFolder', 'activeVersion'].includes(field));
