...
class CreateProject1714133768519 {
    ...
    async alterSharedTable(relationTableName, { escape, runQuery, schemaBuilder: { addForeignKey, addColumns, addNotNull, createIndex, column }, }) {
    ...
    async createUserPersonalProjects({ runQuery, runInBatches, escape }) {
        ...
        await runInBatches(getUserQuery, async (users) => {
            await Promise.all(users.map(async (user) => {
                const projectId = (0, utils_1.generateNanoId)();
    ...
    async down({ logger, escape, runQuery, schemaBuilder: sb }) {
        const { t, c } = escapeNames(escape);
