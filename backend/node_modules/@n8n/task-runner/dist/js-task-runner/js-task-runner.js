...
class JsTaskRunner extends task_runner_1.TaskRunner {
    ...
    async runCode(settings, abortSignal) {
        ...
        try {
            const result = await new Promise((resolve, reject) => {
                ...
                const taskResult = (0, node_vm_1.runInContext)(this.createVmExecutableCode(settings.code), context, { timeout: this.taskTimeout * 1000 });
                void taskResult
                    .then(resolve)
                    .catch(reject)
                    .finally(() => {
    ...
    buildCustomConsole(taskId) {
        return {
            ...JsTaskRunner.CONSOLE_METHODS.reduce((acc, name) => {
