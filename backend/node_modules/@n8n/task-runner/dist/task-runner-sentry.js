...
let TaskRunnerSentry = class TaskRunnerSentry {
    ...
    async initIfEnabled() {
        const { dsn, n8nVersion, environment, deploymentName, profilesSampleRate, tracesSampleRate } = this.config;
        if (!dsn)
            return;
    ...
    isUserCodeError(error) {
        if (error.type === 'EvalError' &&
            error.value === 'Code generation from strings disallowed for this context') {
            return true;
        }
