...
const properties = [
    (0, descriptions_1.modelRLC)('modelSearch'),
    {
        ...
        options: descriptions_1.messageOptions,
    },
    {
    ...
    },
    {
        ...
        options: [
            ...
            descriptions_1.metadataProperty,
            {
...
async function execute(i) {
    ...
    if (externalTools.length) {
        tools = externalTools.length ? externalTools?.map(utils_1.formatToOpenAIResponsesTool) : undefined;
    }
    const body = await responses_1.createRequest.call(this, i, {
        model,
        messages,
        options,
        tools,
    ...
    let response = (await transport_1.apiRequest.call(this, 'POST', '/responses', {
        body,
    }));
    if (body.background) {
        const timeoutSeconds = (0, get_1.default)(options, 'backgroundMode.values.timeout', 300);
        response = await (0, polling_1.pollUntilAvailable)(this, async () => {
            return (await transport_1.apiRequest.call(this, 'GET', `/responses/${response.id}`));
        }, (response) => {
            if (response.error) {
                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Background mode error', {
    ...
    while (toolCalls.length && hasFunctionCall()) {
        ...
        response = (await transport_1.apiRequest.call(this, 'POST', '/responses', {
            body,
        }));
        toolCalls = response.output.filter(isToolRelatedCall);
        currentIteration++;
    }
    const formatType = (0, get_1.default)(body, 'text.format.type');
