...
async function execute(i) {
    ...
    const availableTools = nodeInputs.some((i) => i.type === 'ai_tool')
        ? await (0, helpers_1.getConnectedTools)(this, true)
        : [];
    const tools = [
        {
            functionDeclarations: availableTools.map((t) => ({
                ...
                parameters: {
                    ...(0, zod_to_json_schema_1.default)(t.schema, { target: 'openApi3' }),
        ...
        if (builtInTools) {
            ...
            if (fileSearchOptions) {
                ...
                if (fileSearchStoreNamesRaw) {
                    const parsed = (0, n8n_workflow_1.jsonParse)(fileSearchStoreNamesRaw, {
    ...
    let response = (await transport_1.apiRequest.call(this, 'POST', `/v1beta/${model}:generateContent`, {
        body,
    }));
    ...
    while (toolCalls.length) {
        ...
        response = (await transport_1.apiRequest.call(this, 'POST', `/v1beta/${model}:generateContent`, {
            body,
        }));
