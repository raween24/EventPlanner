        ...
        if (this.getNode().typeVersion >= 1.1 && batchSize > 1) {
            for (let i = 0; i < items.length; i += batchSize) {
                ...
                const batchPromises = batch.map(async (_item, batchItemIndex) => {
                    ...
                    const sentimentCategories = this.getNodeParameter('options.categories', itemIndex, DEFAULT_CATEGORIES);
                    ...
                    if (categories.length === 0) {
                        return {
                            result: null,
                            itemIndex,
                            error: new n8n_workflow_1.NodeOperationError(this.getNode(), 'No sentiment categories provided', {
                                itemIndex,
                            }),
                        };
                    }
				...
				{format_instructions}`);
                    ...
                    try {
                        ...
                        return {
                            result: null,
                            itemIndex,
                            error: new n8n_workflow_1.NodeOperationError(this.getNode(), 'Error during parsing of LLM output, please check your LLM model and configuration', {
                                itemIndex,
                            }),
                        };
                    }
