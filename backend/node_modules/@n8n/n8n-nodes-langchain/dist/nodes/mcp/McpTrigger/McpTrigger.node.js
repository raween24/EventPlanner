...
exports.McpTrigger = void 0;
const McpServer_1 = require("./McpServer");
const error_1 = require("n8n-nodes-base/dist/nodes/Webhook/error");
const utils_1 = require("n8n-nodes-base/dist/nodes/Webhook/utils");
const n8n_workflow_1 = require("n8n-workflow");
const helpers_1 = require("../../../utils/helpers");
const MCP_SSE_SETUP_PATH = 'sse';
const MCP_SSE_MESSAGES_PATH = 'messages';
class McpTrigger extends n8n_workflow_1.Node {
    constructor() {
        super(...arguments);
        this.description = {
    ...
    async webhook(context) {
        ...
        try {
            await (0, utils_1.validateWebhookAuthentication)(context, 'authentication');
        }
        catch (error) {
        ...
        const mcpServer = McpServer_1.McpServer.instance(context.logger);
        if (webhookName === 'setup') {
            const postUrl = node.typeVersion < 2
                ? req.path.replace(new RegExp(`/${MCP_SSE_SETUP_PATH}$`), `/${MCP_SSE_MESSAGES_PATH}`)
                : req.path;
            const connectedTools = await (0, helpers_1.getConnectedTools)(context, true);
            ...
            if (req.method === 'DELETE') {
                ...
                if (sessionId) {
                    const connectedTools = await (0, helpers_1.getConnectedTools)(context, true);
                    const { wasToolCall, toolCallInfo, messageId, relaySessionId, needsListToolsRelay } = await mcpServer.handlePostMessage(req, resp, connectedTools, serverName);
                    if (wasToolCall) {
                        const workflowData = {
                    ...
                    if (needsListToolsRelay && relaySessionId && messageId) {
                        const workflowData = {
                            mcpListToolsRelay: {
                                ...
                                messageId,
                                marker: McpServer_1.MCP_LIST_TOOLS_REQUEST_MARKER,
                            },
                        };
                        return { noWebhookResponse: true, workflowData: [[{ json: workflowData }]] };
                    }
