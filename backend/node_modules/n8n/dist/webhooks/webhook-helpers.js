...
function setupResponseNodePromise(responsePromise, res, responseCallback, workflowStartNode, executionId, workflow) {
    void responsePromise.promise
        ...
        if (binaryData?.id) {
            webhook_response_headers_1.WebhookResponseHeaders.fromObject(response.headers).applyToResponse(res);
...
function prepareExecutionData(executionMode, workflowStartNode, webhookResultData, runExecutionData, runExecutionDataMerge = {}, destinationNode, executionId, workflowData) {
    ...
    if (workflowStartNode.type === n8n_workflow_1.MICROSOFT_AGENT365_TRIGGER_NODE_TYPE &&
        runExecutionData?.executionData?.nodeExecutionStack) {
        (0, merge_1.default)(runExecutionData.executionData.nodeExecutionStack, nodeExecutionStack);
    }
...
async function executeWebhook(workflow, webhookData, workflowData, workflowStartNode, executionMode, pushRef, runExecutionData, executionId, req, res, responseCallback, destinationNode) {
    ...
    try {
        ...
        if ([constants_1.MCP_TRIGGER_NODE_TYPE, n8n_workflow_1.MICROSOFT_AGENT365_TRIGGER_NODE_TYPE].includes(workflowStartNode.type)) {
            const nodeExecutionStack = [];
        ...
        executionId = await di_1.Container.get(workflow_runner_1.WorkflowRunner).run(runData, true, !didSendResponse && !shouldDeferOnReceivedResponse, executionId, responsePromise);
        ...
        if (shouldDeferOnReceivedResponse) {
            ...
            const evaluatedResponseData = workflow.expression.getComplexParameterValue(workflowStartNode, webhookData.webhookDescription.responseData, executionMode, additionalKeys, undefined, 'firstEntryJson');
        ...
        if (WorkflowHelpers.shouldRestartParentExecution(parentExecution)) {
            ...
            void executePromise
                ...
                if (subworkflowResults.status === 'waiting')
                    return;
                await WorkflowHelpers.updateParentExecutionWithChildResults(executionRepository, parentExecution.executionId, subworkflowResults);
...
function evaluateResponseHeaders(context) {
    const headers = new webhook_response_headers_1.WebhookResponseHeaders();
