...
let LiveWebhooks = class LiveWebhooks {
    ...
    async executeWebhook(request, response) {
        ...
        if (!workflowData.activeVersion) {
            throw new not_found_error_1.NotFoundError(`Active version not found for workflow with id "${webhook.workflowId}"`);
        }
        ...
        const ownerProjectId = workflowData.shared.find((share) => share.role === 'workflow:owner')?.projectId;
        ...
        return await new Promise((resolve, reject) => {
            ...
            void WebhookHelpers.executeWebhook(workflow, webhookData, activeWorkflowData, workflowStartNode, executionMode, undefined, undefined, undefined, request, response, async (error, data) => {
                if (error !== null) {
                    return reject(error);
                }
