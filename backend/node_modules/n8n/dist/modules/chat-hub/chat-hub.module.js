...
let ChatHubModule = class ChatHubModule {
    async init() {
        ...
        const { ChatHubEventRelay } = await Promise.resolve().then(() => __importStar(require('./chat-hub-event-relay.service')));
        di_1.Container.get(ChatHubEventRelay);
        const isQueueMode = di_1.Container.get(config_1.ExecutionsConfig).mode === 'queue';
        const isWorker = di_1.Container.get(n8n_core_1.InstanceSettings).isWorker;
        if (!isQueueMode || isWorker) {
            await Promise.resolve().then(() => __importStar(require('./chat-hub-execution-watcher.service')));
        }
    }
    async settings() {
        const { ChatHubSettingsService } = await Promise.resolve().then(() => __importStar(require('./chat-hub.settings.service')));
        const service = di_1.Container.get(ChatHubSettingsService);
    ...
    async entities() {
        ...
        const { ChatHubTool } = await Promise.resolve().then(() => __importStar(require('./chat-hub-tool.entity')));
