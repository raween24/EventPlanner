...
const execution_persistence_1 = require("./execution-persistence");
const execution_redaction_proxy_service_1 = require("./execution-redaction-proxy.service");
...
let ExecutionService = class ExecutionService {
    constructor(globalConfig, logger, activeExecutions, executionAnnotationRepository, annotationTagMappingRepository, executionRepository, executionPersistence, workflowRepository, nodeTypes, waitTracker, workflowRunner, concurrencyControl, license, workflowSharingService, eventService, executionRedactionServiceProxy) {
        ...
        this.executionRedactionServiceProxy = executionRedactionServiceProxy;
    }
    async findOne(req, sharedWorkflowIds) {
        if (!sharedWorkflowIds.length)
            return undefined;
        const { id: executionId } = req.params;
        const execution = await this.executionRepository.findIfSharedUnflatten(executionId, sharedWorkflowIds);
        if (!execution) {
        ...
        const processedExecution = await this.executionRedactionServiceProxy.processExecution(execution, {
            user: req.user,
            redactExecutionData,
            ipAddress: req.ip ?? '',
        ...
        return {
            ...execution,
            data: (0, flatted_1.stringify)(processedExecution.data),
        };
    }
    async getLastSuccessfulExecution(workflowId, user, redactExecutionData) {
    ...
    async stop(executionId, sharedWorkflowIds) {
        const execution = await this.executionRepository.findWithUnflattenedData(executionId, sharedWorkflowIds);
        if (!execution) {
            this.logger.info(`Unable to stop execution "${executionId}" as it was not found or not accessible`, {
                executionId,
            });
    ...
    async stopMany(query, sharedWorkflowIds) {
        ...
        for (const { id } of executions) {
            try {
                ...
                if (!(e instanceof missing_execution_stop_error_1.MissingExecutionStopError)) {
                    this.logger.warn(`Unexpected error while attempting to stop execution ${id}: ${(0, n8n_workflow_1.ensureError)(e).message}`);
                }
            }
        }
        return stopped;
...
exports.ExecutionService = ExecutionService = __decorate([
    ...
    __metadata("design:paramtypes", [config_1.GlobalConfig,
        ...
        execution_redaction_proxy_service_1.ExecutionRedactionServiceProxy])
], ExecutionService);
