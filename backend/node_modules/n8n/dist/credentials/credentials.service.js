...
let CredentialsService = class CredentialsService {
    ...
    async getMany(user, { listQueryOptions = {}, includeScopes = false, includeData = false, onlySharedWithMe = false, includeGlobal = false, externalSecretsStore, } = {}) {
        ...
        let credentials;
        if (returnAll) {
            credentials = await this.getManyForAdminUser(user, listQueryOptions, includeGlobal, includeData, onlySharedWithMe);
        }
        else {
            credentials = await this.getManyForMemberUser(user, listQueryOptions, includeGlobal, includeData, onlySharedWithMe);
        }
    ...
    filterByExternalSecretsStore(credentials, externalSecretsStore) {
        ...
        credentials = credentials.filter((credential) => {
            ...
            const matchingSecretPaths = (0, utils_1.getAllKeyPaths)(decryptedData, '', [], (value) => {
    ...
    async getManyForAdminUser(user, listQueryOptions, includeGlobal, includeData, onlySharedWithMe) {
        if (onlySharedWithMe) {
            const sharingOptions = {
                onlySharedWithMe: true,
            };
            const { credentials } = await this.credentialsRepository.getManyAndCountWithSharingSubquery(user, sharingOptions, listQueryOptions);
            if (includeGlobal) {
    ...
    async getManyForMemberUser(user, listQueryOptions, includeGlobal, includeData, onlySharedWithMe) {
    ...
    async createCredential(opts, user) {
        await this.checkCredentialData(opts.type, opts.data, user, opts.projectId ?? '');
        if (this.externalSecretsConfig.externalSecretsForProjects && opts.projectId) {
            await (0, validation_1.validateAccessToReferencedSecretProviders)(opts.projectId, opts.data, this.externalSecretsProviderAccessCheckService, 'create');
        }
