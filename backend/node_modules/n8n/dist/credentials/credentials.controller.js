...
let CredentialsController = class CredentialsController {
    constructor(globalConfig, credentialsService, enterpriseCredentialsService, namingService, licenseState, logger, userManagementMailer, sharedCredentialsRepository, projectRelationRepository, eventService, credentialsFinderService) {
    ...
    async getOne(req, _res, credentialId, query) {
        const { shared, ...credential } = this.licenseState.isSharingLicensed()
            ? await this.enterpriseCredentialsService.getOneForUser(req.user, credentialId, query.includeData)
    ...
    async testCredentials(req) {
        ...
        const storedCredential = await this.credentialsFinderService.findCredentialForUser(credentials.id, req.user, ['credential:read']);
        if (!storedCredential) {
    ...
    async updateCredentials(req) {
        ...
        const credential = await this.credentialsFinderService.findCredentialForUser(credentialId, user, ['credential:update']);
        if (!credential) {
        ...
        const preparedCredentialData = await this.credentialsService.prepareUpdateData(req.user, req.body, credential);
    ...
    async deleteCredentials(req) {
        ...
        const credential = await this.credentialsFinderService.findCredentialForUser(credentialId, req.user, ['credential:delete']);
        if (!credential) {
    ...
    async shareCredentials(req) {
        ...
        if (toShare.length > 0) {
            const canShare = await (0, check_access_1.userHasScopes)(req.user, ['credential:share'], false, {
                credentialId,
            });
            if (!canShare) {
                throw new forbidden_error_1.ForbiddenError();
        ...
        if (toUnshare.length > 0) {
            const canUnshare = await (0, check_access_1.userHasScopes)(req.user, ['credential:unshare'], false, {
                credentialId,
            });
            if (!canUnshare) {
                throw new forbidden_error_1.ForbiddenError();
