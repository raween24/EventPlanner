...
let ExternalHooks = class ExternalHooks {
    constructor(logger, errorReporter, globalConfig, userRepository, settingsRepository, credentialsRepository, workflowRepository) {
        this.logger = logger;
    ...
    async run(hookName, hookParameters) {
        ...
        for (const hookFunction of hookFunctions) {
            try {
                ...
                const error = new n8n_workflow_1.UnexpectedError(`External hook "${hookName}" failed`, { cause });
