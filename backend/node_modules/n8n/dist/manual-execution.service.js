    ...
    runManually(data, workflow, additionalData, executionId, pinData) {
        if (data.triggerToStartFrom?.data && data.startNodes?.length) {
            ...
            if (data.destinationNode) {
                ...
                a.ok(destinationNode, `Could not find a node named "${data.destinationNode.nodeName}" in the workflow.`);
                ...
                if (n8n_workflow_1.NodeHelpers.isTool(destinationNodeType.description, destinationNode.parameters)) {
                    ...
                    data.destinationNode = { nodeName: constants_1.TOOL_EXECUTOR_NODE_NAME, mode: 'inclusive' };
                    const connectedTools = workflow.getParentNodes(constants_1.TOOL_EXECUTOR_NODE_NAME, 'ALL_NON_MAIN');
            ...
            a.ok(data.destinationNode, 'a destinationNodeName is required for the new partial execution flow');
