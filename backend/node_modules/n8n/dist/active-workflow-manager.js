...
let ActiveWorkflowManager = class ActiveWorkflowManager {
    constructor(logger, errorReporter, activeWorkflows, activeExecutions, externalHooks, nodeTypes, webhookService, workflowRepository, activationErrorsService, executionService, workflowStaticDataService, activeWorkflowsService, workflowExecutionService, instanceSettings, publisher, workflowsConfig, push, eventService, storageConfig) {
        this.logger = logger;
    ...
    async clearWebhooks(workflowId) {
        ...
        if (!workflowData.activeVersion) {
            throw new n8n_workflow_1.UnexpectedError('Active version not found for workflow', {
                extra: { workflowId },
            });
        }
    ...
    getExecuteTriggerFunctions(workflowData, additionalData, mode, activation) {
        return (workflow, node) => {
            ...
            const saveFailedExecution = (error) => {
                this.logger.info(`The trigger node "${node.name}" of workflow "${workflowData.name}" reported the error: "${error.message}". Saving to failed executions`, {
                    nodeName: node.name,
                    workflowId: workflowData.id,
                    workflowName: workflowData.name,
                });
            ...
            return new n8n_core_1.TriggerContext(workflow, node, additionalData, mode, activation, emit, emitError, saveFailedExecution);
        };
    }
    ...
    async add(workflowId, activationMode, existingWorkflow, { shouldPublish } = { shouldPublish: true }) {
        ...
        if (this.instanceSettings.isMultiMain && shouldPublish) {
            if (!dbWorkflow?.activeVersionId) {
                throw new n8n_workflow_1.UnexpectedError('Active version ID not found for workflow', {
                    extra: { workflowId },
                });
            }
        ...
        try {
            ...
            const validation = (0, n8n_workflow_1.validateWorkflowHasTriggerLikeNode)(workflow.nodes, this.nodeTypes, constants_1.STARTING_NODES);
    ...
    handleDisplayWorkflowActivation({ workflowId, activeVersionId, }) {
