...
let TaskBrokerWsServer = class TaskBrokerWsServer {
    constructor(logger, taskBroker, disconnectAnalyzer, taskRunnersConfig, runnerLifecycleEvents, globalConfig) {
        this.logger = logger;
        ...
        this.heartbeatTimer = setInterval(() => {
            for (const [runnerId, connection] of this.runnerConnections.entries()) {
                if (!connection.isAlive) {
                    void this.removeConnection(runnerId, 'failed-heartbeat-check', constants_2.WsStatusCodes.CloseProtocolError);
    ...
    async stopConnectedRunners() {
        ...
        await Promise.all(Array.from(this.runnerConnections.keys()).map(async (id) => await this.removeConnection(id, 'shutting-down', constants_2.WsStatusCodes.CloseGoingAway)));
    }
