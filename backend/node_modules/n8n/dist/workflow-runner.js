...
let WorkflowRunner = class WorkflowRunner {
    constructor(logger, errorReporter, activeExecutions, executionRepository, workflowStaticDataService, nodeTypes, credentialsPermissionChecker, instanceSettings, manualExecutionService, failedRunFactory, eventService, executionsConfig, storageConfig, externalHooks) {
        this.logger = logger;
    ...
    async run(data, loadStaticData, realtime, restartExecutionId, responsePromise) {
        ...
        try {
            ...
            const runData = this.failedRunFactory.generateFailedExecutionFromError(data.executionMode, error, error.node);
        ...
        if (shouldEnqueue) {
            await this.enqueueExecution(executionId, workflowId, data, loadStaticData, realtime, restartExecutionId);
        }
        else {
    ...
    async enqueueExecution(executionId, workflowId, data, loadStaticData, realtime, restartExecutionId) {
        const jobData = {
            workflowId,
            executionId,
    ...
    needsFullExecutionData(executionMode, executionId) {
        if (!process.env.N8N_MINIMIZE_EXECUTION_DATA_FETCHING)
            return true;
        return (executionMode === 'integrated' ||
