...
function createLazySwaggerMiddleware(openApiSpecPath, publicApiEndpoint, version) {
    ...
    return async (req, res, next) => {
        if (!cachedRouter) {
            ...
            const { default: YAML } = await Promise.resolve().then(() => __importStar(require('yamljs')));
        ...
        if (!cachedRouter) {
            initPromise ??= (async () => {
                ...
                router.use(openApiValidatorMiddleware({
                    ...
                    validateSecurity: {
                        handlers: {
                            ApiKeyAuth: di_1.Container.get(public_api_key_service_1.PublicApiKeyService).getAuthMiddleware(version),
                        },
                    },
                }));
                return router;
